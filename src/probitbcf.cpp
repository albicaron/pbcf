...
// [[Rcpp::export]]
List probitbcf(arma::vec y,       // True latent variable values of the response.
              ...
              CharacterVector treef_name_="pbcf_trees.txt",
			  ...
			  double trt_init = 1.0)
{
   ...
   for(size_t i=0;i<ntree_con;i++) t_con[i].setm(ybar / ntree_con);
   ...
   for (size_t i = 0; i < n; i++) allfit_mod[i] = (z[i] * bscale1 + (1 - z[i])*bscale0)*trt_init;
   ...
   for(size_t i=0;i<(nsim+nburn);i++) {
      ...
      //------------------------------------------------------------------------
      //draw treatment trees for [(alpha_tau + eta_tau * mu(x) ) * bscale]
      //------------------------------------------------------------------------
      ...
      if(i>=nburn) {
         ...
         if(di_mod_pred.n>0){
            for(size_t k=0;k<n_pred;k++) {
               ...
               yhat_est_post(i-nburn,k) = pi_con.eta * fit_i(k,t_con,xi_con,di_con_pred) + tau_temp*bscale;
